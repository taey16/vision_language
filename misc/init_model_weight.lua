...
function nn_init.MSRinit(net)
  local function Convinit(module_type)
    for k, v in pairs(net:findModules(module_type)) do
      ...
      print(string.format('Convinit Kaiming (is bias nil? %s', tostring(v.bias == nil)))
    end
  end
  local function BNinit(module_type)
    for k,v in pairs(net:findModules(module_type)) do
      print(string.format('BNinit: weight %f (is bias nil? %s', opt.init_gamma, tostring(v.bias == nil)))
  ...
  local function Linearinit(module_type)
    for k,v in pairs(net:findModules(module_type)) do
      print(string.format(
        'Linearinit: bias 0.0 (is bias nil? %s)', tostring(v.bias == nil)))
      if v.bias then
        v.bias:zero()
      end
    end
