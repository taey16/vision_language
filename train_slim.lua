...
local function eval_split(split, evalopt)
  ...
  while true do
    ...
    data.images = net_utils.preprocess(data.images, opt.crop_size, false, false)
    ...
    if verbose then
      io.flush(print(string.format(
        'evaluating validation performance... %d/%d (loss: %f)', 
        ix0-1, ix1, loss)))
    end
...
while true do  
  ...
  if smooth_loss == 0 then 
    smooth_loss = losses.total_loss
  else
    smooth_loss = 0.99 * smooth_loss + 0.01 * losses.total_loss
  end
  ...
  if iter % opt.display == 0 then
    ...
    io.flush(print(string.format(
      '%d/%d: %.2f, loss: %f, smooth-loss: %f, lr: %.8f, cnn_lr: %.8f, finetune: %s, optim: %s, elapsed: %.3f', 
      iter, number_of_batches, epoch,
      losses.total_loss, smooth_loss,
      learning_rate, cnn_learning_rate, 
      tostring(finetune_cnn), opt.optim, elapsed_trn)))
  end

  -- save checkpoint once in a while (or on final iteration)
  if (iter == 0 and opt.test_initialization == true) or
     ((iter+1) % opt.save_checkpoint_every == 0 or (iter+1) == opt.max_iters) then
    logger_trn:add{
      ['time'] = elapsed_trn,
      ['iter'] = iter,
      ['epoch'] = epoch,
    ...
    print(string.format('validation loss: %f', val_loss))
    io.flush()
    ...
    logger_tst:add{['time'] = elapsed_tst,
                   ['iter'] = iter,
                   ['epoch']= epoch,
                   ['loss'] = val_loss}
  end
